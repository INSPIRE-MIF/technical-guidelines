...
The xml schema document is available on the INSPIRE website _https://inspire.ec.europa.eu/schemas/ge_hg/4.0/HydrogeologyCore.xsd_
...
The xml schema document is available on the INSPIRE website _https://inspire.ec.europa.eu/schemas/ge_gp/4.0/GeophysicsCore.xsd_
...
===== The use of inspire.ec.europa.eu/schemas/ge-core/4.0/GeologyCore.xsd encoding

*Name: Geology GML Application Schema*
Version: 4.0,
Specification: D2.8.II.4 Data Specification on _Geology_ – Technical Guidelines Character set: UTF-8

The xml schema document is available on the INSPIRE website
_https://inspire.ec.europa.eu/schemas/ge-core/4.0/GeologyCore.xsd_
...
{empty}[SERVICE / ROOT LAYER NAME]
{empty}-> [ROOT LAYER TITLE]
{empty}- - + - - + [GROUP LAYER NAME ~ follows data specification naming requirement]
{empty}->-> [GROUP LAYER TITLE ~ follows data specification naming requirement]
{empty}- - + - - + - - + [LAYER NAME ~ layer of the data specification TYPE BUT following community convention]
...
 
For example:
 
{empty}- - + BGS_EN_Bedrock_and_Surface_Geology
{empty}-> BGS OGE bedrock and surface geology
{empty} - - + - - + GE.GeologicFault
{empty}->-> Geologic Faults
{empty}ABSTRACT: MappedFeature (spatial objects whose specification property is of type ShearDisplacementStructure)
{empty}- - + - - + - - + GE.GeologicFault.BGS.EN.1M.Surface
{empty}- - + - - + - - + GE.GeologicFault.BGS.EN.1M.Bedrock
{empty}- - + - - + GE.GeologicUnit
{empty}->-> Geologic Units
{empty}ABSTRACT: MappedFeature (spatial objects whose specification property is of type GeologicUnit)
{empty}- - + - - + - - + GE.GeologicUnit.BGS.EN.1M.Surface.Lithology
{empty}- - + - - + - - + GE.GeologicUnit.BGS.EN.1M.Surface.Age
{empty}- - + - - + - - + GE.GeologicUnit.BGS.EN.1M.Bedrock.Lithology
{empty}- - + - - + - - + GE.GeologicUnit.BGS.EN.1M.Bedrock.Age
...
|===
|*Style Name* |GE.Borehole
|*Style Title* |Borehole Depth Type
|*Style Abstract* |The colour ramp is to differentiate based on *BoreholeLength* measure of a borehole depth.
...
[DS-D2.5] INSPIRE DS-D2.5, Generic Conceptual Model, v3.4rc2, _https://knowledge-base.inspire.ec.europa.eu/publications/inspire-generic-conceptual-model_en_
...
[DS-D2.7] INSPIRE DS-D2.7, Guidelines for the encoding of spatial data, v3.3rc2, _https://knowledge-base.inspire.ec.europa.eu/publications/guidelines-encoding-spatial-data_en_
