...
|*Rights* |{copyright}
|*Identifier* |https://inspire.ec.europa.eu/id/document/tg/el[D2.8.II.1_v3.3.0]
|*Changelog* |{revremark}
|*Language* |{lang}
...
* The ElevationBaseTypes application schema, which provides basic codelists for the rest application schemas of _Elevation_ (Section 5.4).
...
+
_ElevationGridCoverage_, _ElevationVectorElements_ and _ElevationTIN_ application schemas have dependency relationships to the _ElevationBaseTypes_ application schema, since they use generic codelists defined in it.
...
The _ElevationBaseTypes_ application schema provides the basic framework for the _Elevation_ theme. Regardless of the spatial representation type used for the elevation data, it defines codelist which are common to the other _Elevation_ application schemas: _ElevationGridCoverage_, _ElevationVectorElements_ and _ElevationTIN_.

====== ElevationPropertyTypeValue codelist
...
|Definition: |Code list type which determines the elevation property which has been measured or calculated.
...
|Extensibility: |none
|Identifier: |http://inspire.ec.europa.eu/codelist/ElevationPropertyType
|Value: |The allowed values for this code list comprise only the values specified in the INSPIRE Registry.
|===
...
|Definition: |Code list type which determines the elevation surface with regard to its relative adherence to the Earth's bare surface.
|Description: |EXAMPLE DTM and DSM.
|Extensibility: |none
|Identifier |http://inspire.ec.europa.eu/codelist/SurfaceTypeValue
|Value: |The allowed values for this code list comprise only the values specified in the INSPIRE Registry.
|===
...
|Values: |The allowed values for this code list comprise the values specified in INSPIRE Registry and narrower values defined by data providers.
|===
...
|Extensibility: |none
|Identifier: |http://inspire.ec.europa.eu/codelist/CountourLineTypeValue
|Value: |The allowed values for this code list comprise only the values specified in the INSPIRE Registry.
|===
...
The xml schema document is available on the INSPIRE website _https://inspire.ec.europa.eu/schemas/el-cov/5.0/ElevationGridCoverage.xsd_
...
The xml schema document is available on the INSPIRE website https://inspire.ec.europa.eu/schemas/el-vec/5.0/ElevationVectorElements.xsd
...
The xml schema document is available on the INSPIRE website https://inspire.ec.europa.eu/schemas/el-tin/5.0/ElevationTin.xsd
